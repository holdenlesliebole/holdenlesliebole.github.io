...
than reading yesterday's gauge at 1-day lead, it does **not** transfer to an unseen
hydrologic regime, its uncertainty estimates are **not yet calibrated**, and its win over
the operational process model **depends on which version of that model, and which years,
you test against.**
...
and the 2023 AR sequence. Training uses every other day in the record; validation is
2009–2011. Checkpoints are selected on validation, not taken from the last epoch. Of the
28 trained basins, 22 have observations in the flood windows; every median below is over
that fixed cohort.

**The test-period choice is the first result.** An earlier version of this work tested on
2012–2014 and reported median NSE 0.81. That window is California drought onset. On the
same 22 basins, the drought-window model scores **0.836** and the flood-year model scores
**0.754**. The original number wasn't wrong; it was measured on a benign period. Anyone
reporting streamflow skill without saying which hydrologic years they tested on is
reporting the years, not the model.
...
| Lead (days) | LSTM | Gauge persistence | LSTM, 5 snowmelt basins | Persistence, 5 snowmelt basins |
|---|---|---|---|---|
| 1 | 0.667 | **0.712** | 0.810 | **0.944** |
| 2 | **0.688** | 0.484 | 0.810 | **0.864** |
| 3 | **0.665** | 0.301 | **0.809** | 0.803 |
| 5 | **0.670** | 0.037 | **0.802** | 0.702 |
| 7 | **0.597** | −0.122 | **0.792** | 0.610 |

(Train-period mean scores −0.08; day-of-year climatology +0.20. Neither is competitive.)

**At one day ahead, yesterday's gauge reading beats the model nearly everywhere.**
Persistence decays with lead; the LSTM barely does. They cross at **day 2** across the
cohort and at **day 3** on the high-storage snowmelt basins this project was built
around — and on the rain-driven basin the LSTM wins at every lead (0.618 vs 0.541 at
day 1, persistence negative by day 3), because flashy rain response isn't persistent.

**One asymmetry must travel with these numbers:** the LSTM never ingests observed
discharge. Its inputs are precipitation and temperature only. Persistence uses a gauge
reading the model is denied — but a real flood operator *has* that gauge, which is exactly
why the comparison belongs here. It also points at the obvious next architecture:
assimilate the gauge.


<figure>
  <img src="{{ '/assets/notes/cv_persistence_crossover.png' | relative_url }}" alt="Median NSE by forecast lead. Persistence decays; the LSTM holds nearly flat. They cross at day 2 across the cohort and day 3 on the snowmelt focus basins." />
  <figcaption>Persistence decays with lead while the LSTM stays nearly flat; the curves cross at day 2 to 3.</figcaption>
</figure>
...
| **High-flow volume bias (FHV)** | **−18%** |
| **Peaks missed** | **~47%** |
| Peak magnitude error | ~49% |
...
<figure>
  <img src="{{ '/assets/notes/cv_flood_hydrographs.png' | relative_url }}" alt="Observed vs simulated hydrographs for the five focus basins over water year 2017; simulated peaks arrive on time but consistently lower than observed." />
  <figcaption>Held-out WY2017: the freshet and storm timing are captured; the peaks are consistently short.</figcaption>
</figure>

This is not an ML-specific failure. On a separately held-out extreme (January 1997, with
the entire 1990s excluded from training), **both the LSTM and the operational NWM
...
average skill (NSE 0.679 → 0.754) and made peaks *worse* (FHV −14% → −18%, missed peaks
0.39 → 0.47). More capacity buys accuracy on the bulk of the distribution and costs
...
<figure>
  ...
  <figcaption>The same basin, two models: trained on it (blue) vs held out of training (purple). The held-out model applies snowmelt physics to a rain basin and invents peaks.</figcaption>
</figure>

Trained on 27 mostly-snowmelt catchments, the model learned snowmelt. Asked about a
rain-driven catchment it had never seen, it confidently applied the wrong physics.
...
The diagnosis comes from an apparent contradiction: coverage says the intervals are too
narrow, while spread/skill ≈ 3 says the ensemble standard deviation is nearly three times
too large. Both hold only if the predictive distribution is **heavy-tailed** — variance
inflated by rare extreme samples while the central interval stays too concentrated.

Calibration is the most fixable limitation here — post-hoc variance inflation needs no
retraining, and fewer mixture components or a Gaussian mixture head are known remedies. The
open question is whether calibration can be recovered without surrendering the CRPS gain.
...
On WY2017, the **modern NWM beats the LSTM on average skill and is far better on high-flow
bias and peak error** — a large share of the original margin was the NWM *version*, not the
physics. On the partial WY2023 window (the v3.0 retrospective ends January 2023, so this
covers the December–January AR sequence but not the March storms) the LSTM wins on average
skill, and both systems underestimate high flows by ~50%.


<figure>
  <img src="{{ '/assets/notes/cv_lstm_vs_nwm_flood.png' | relative_url }}" alt="Grouped bar chart of NSE per focus basin: NWM v3.0 leads most basins in WY2017 except Bear Creek; the LSTM leads most basins in the partial WY2023 window except Mill Creek." />
  <figcaption>Per-basin NSE on the flood windows. NWM v3.0 wins WY2017 except at Bear Ck; the LSTM wins the WY2023 window except at rain-driven Mill Ck.</figcaption>
</figure>
...
| Use case | Verdict |
|---|---|
| 2–7 day volume forecast, gauged basin, regime seen in training | **Yes** — beats persistence from day 2–3 and holds skill where it collapses |
| 1-day forecast | **No** — the gauge is better, and free |
...
| Replacing the operational model | **No** — v3.0 wins WY2017 outright; the models are complementary by regime |
...
uncertainty below 0.001 NSE, so reported differences are not sampling noise.

That protocol exists because the closure tests kept catching real problems: a test-period
choice that flattered skill by 0.08 NSE, an evaluation code path that silently dropped a
fifth of the basins from its medians, and an upstream bug that writes corrupted
